...
[![Standards Readiness](https://img.shields.io/badge/Standards--Readiness-72.2%2F100-yellow)](docs/standards-readiness.md)
...
**AIIR records what's declared, not what's hidden.** Detection is heuristic, not magic.

We built this repo almost entirely with Copilot Chat (Claude and ChatGPT models). Of our 246 dogfood receipts, **195 are attested `human`** — because Copilot Chat doesn't add `Co-authored-by` trailers. Only 9 have explicit AI markers; 42 are correctly classified as `bot` (Dependabot, GitHub Actions). Those 195 false negatives aren't a bug; they're the gap AIIR exists to close.

**What it catches:**

- `Co-authored-by: Copilot` or similar trailers
...
- Git trailers: `Generated-by:`, `AI-assisted:`, `AI-model:`, `Tool:`
- Unicode evasion of declared signals (TR39 confusable resolution, Cf stripping, NFKC normalization)

**What it doesn't catch:**

- **Agentic coding sessions** — Copilot Chat, Claude Code, ChatGPT Canvas, Cursor Agent, and similar agent-mode tools don't add `Co-authored-by` trailers. A commit authored entirely by an AI agent looks identical to a human commit in git metadata.
- **Copilot inline completions** — accepted completions leave no metadata trace by default
- **Copy-paste from ChatGPT/Claude** — no attribution unless the user adds it
- **Squash-merged AI branches** — clean squash messages lose per-commit trailers
- **Amended commits** — `git commit --amend` can remove AI trailers after the fact

**What's coming:** The receipt *format* is complete — content-addressed, deterministic, CBOR round-trip, Sigstore-signed. The detection layer will improve as the ecosystem adopts richer commit metadata:

- **Editor plugins** (VS Code extension) that report active AI sessions via commit trailers
