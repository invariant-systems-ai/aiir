...
_RE_SHA_HEX = re.compile(r"^[0-9a-f]{40}$|^[0-9a-f]{64}$")
_RE_HASH_PREFIX = re.compile(r"^sha256:[0-9a-f]{64}$")
_RE_VERSION = re.compile(r"^[0-9]+\.[0-9]+\.[0-9]+([.+\-][0-9a-zA-Z.+\-]*)?$")
...
def validate_receipt_schema(receipt: Any) -> List[str]:
    ...
    if not _RE_VERSION.match(receipt["version"]):
        errors.append(
            f"version must be SemVer (e.g. '1.0.14'), got {receipt['version']!r}"
        )
...
def _validate_commit(commit: Dict[str, Any], errors: List[str]) -> None:
    ...
    if isinstance(msg_hash, str) and not _RE_HASH_PREFIX.match(msg_hash):
        errors.append("commit.message_hash must match 'sha256:' + 64 hex chars")

    diff_hash = commit.get("diff_hash")
    if isinstance(diff_hash, str) and not _RE_HASH_PREFIX.match(diff_hash):
        errors.append("commit.diff_hash must match 'sha256:' + 64 hex chars")
    ...
    if has_files:
        ...
        if not isinstance(files, list):
            ...
            errors.append(
                f"commit.files must have at most 100 entries, got {len(files)}"
            )
        else:
...
def _validate_ai_attestation(ai: Dict[str, Any], errors: List[str]) -> None:
    ...
    if "bot_signals_detected" in ai:
        ...
        if not isinstance(bsigs, list):
            ...
            for i, s in enumerate(bsigs):
                if not isinstance(s, str):
                    errors.append(
                        f"{prefix}.bot_signals_detected[{i}] must be a string"
                    )
                    break
    ...
    if "bot_signal_count" in ai:
        ...
        elif "bot_signals_detected" in ai and isinstance(
            ai["bot_signals_detected"], list
        ):
    ...
    if "authorship_class" in ai:
        ...
        # Canonical forms (SPEC §4 + schema enum).
        # Legacy hyphenated forms accepted for backward compatibility
        # with receipts generated by v1.0.4–v1.0.13.
        valid_classes = {
            "human",
            "ai_assisted",
            "ai-assisted",
